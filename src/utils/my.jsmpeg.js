const MyJsmpeg = {
    ...
    CreateVideoElements: function () {
        ...
        for (var i = 0; i < elements.length; i++) {
            new MyJsmpeg.VideoElement(elements[i])
        }
    },
...
if (document.readyState === "complete") {
    MyJsmpeg.CreateVideoElements()
} else {
    document.addEventListener("DOMContentLoaded", MyJsmpeg.CreateVideoElements)
}
MyJsmpeg.VideoElement = function () {
    "use strict";
    var VideoElement = function (element) {
        ...
        this.player = new MyJsmpeg.Player(url, options);
...
MyJsmpeg.Player = function () {
    ...
    var Player = function (url, options) {
        ...
        if (options.source) {
            ...
            this.source = new MyJsmpeg.Source.WebSocket(url, options);
            options.streaming = true
        } else if (options.progressive !== false) {
            this.source = new MyJsmpeg.Source.AjaxProgressive(url, options);
            options.streaming = false
        } else {
            this.source = new MyJsmpeg.Source.Ajax(url, options);
            options.streaming = false
        }
        this.maxAudioLag = options.maxAudioLag || .25;
        this.loop = options.loop !== false;
        this.autoplay = !!options.autoplay || options.streaming;
        this.demuxer = new MyJsmpeg.Demuxer.TS(options);
        this.source.connect(this.demuxer);
        if (!options.disableWebAssembly && MyJsmpeg.WASMModule.IsSupported()) {
            this.wasmModule = new MyJsmpeg.WASMModule;
            options.wasmModule = this.wasmModule
        }
        if (options.video !== false) {
            this.video = options.wasmModule ? new MyJsmpeg.Decoder.MPEG1VideoWASM(options) : new MyJsmpeg.Decoder.MPEG1Video(options);
            this.renderer = !options.disableGl && MyJsmpeg.Renderer.WebGL.IsSupported() ? new MyJsmpeg.Renderer.WebGL(options) : new MyJsmpeg.Renderer.Canvas2D(options);
            this.demuxer.connect(MyJsmpeg.Demuxer.TS.STREAM.VIDEO_1, this.video);
            this.video.connect(this.renderer)
        }
        if (options.audio !== false && MyJsmpeg.AudioOutput.WebAudio.IsSupported()) {
            this.audio = options.wasmModule ? new MyJsmpeg.Decoder.MP2AudioWASM(options) : new MyJsmpeg.Decoder.MP2Audio(options);
            this.audioOut = new MyJsmpeg.AudioOutput.WebAudio(options);
            this.demuxer.connect(MyJsmpeg.Demuxer.TS.STREAM.AUDIO_1, this.audio);
        ...
        if (this.wasmModule) {
            if (MyJsmpeg.WASM_BINARY_INLINED) {
                var wasm = MyJsmpeg.Base64ToArrayBuffer(MyJsmpeg.WASM_BINARY_INLINED);
    ...
    Player.prototype.seek = function (time) {
        ...
        this.startTime = MyJsmpeg.Now() - time
    };
    ...
    Player.prototype.update = function () {
        ...
        if (!this.isPlaying) {
            this.isPlaying = true;
            this.startTime = MyJsmpeg.Now() - this.currentTime;
    ...
    Player.prototype.updateForStaticFile = function () {
        ...
        if (this.audio && this.audio.canPlay) {
            ...
            var targetTime = MyJsmpeg.Now() - this.startTime + this.video.startTime,
...
MyJsmpeg.Source.AjaxProgressive = function () {
    "use strict";
    ...
    AjaxProgressiveSource.prototype.loadNextChunk = function () {
        ...
        this.isLoading = true;
        this.loadStartTime = MyJsmpeg.Now();
        this.request = new XMLHttpRequest;
        this.request.onreadystatechange = function () {
    ...
    AjaxProgressiveSource.prototype.onChunkLoad = function (data) {
        ...
        this.loadTime = MyJsmpeg.Now() - this.loadStartTime;
        if (!this.throttled) {
            this.loadNextChunk()
        }
    };
...
MyJsmpeg.Source.WebSocket = function () {
    "use strict";
...
MyJsmpeg.Demuxer.TS = function () {
    ...
    TS.prototype.write = function (buffer) {
        if (this.leftoverBytes) {
            ...
            this.bits = new MyJsmpeg.BitBuffer(totalLength);
            this.bits.write([this.leftoverBytes, buffer])
        } else {
            this.bits = new MyJsmpeg.BitBuffer(buffer)
        }
    ...
    TS.prototype.resync = function () {
        ...
        console.warn("MyJsmpeg: Possible garbage data. Skipping.");
...
MyJsmpeg.Decoder.Base = function () {
    "use strict";
...
MyJsmpeg.Decoder.MPEG1Video = function () {
    "use strict";
    var MPEG1 = function (options) {
        MyJsmpeg.Decoder.Base.call(this, options);
        this.onDecodeCallback = options.onVideoDecode;
        var bufferSize = options.videoBufferSize || 512 * 1024;
        var bufferMode = options.streaming ? MyJsmpeg.BitBuffer.MODE.EVICT : MyJsmpeg.BitBuffer.MODE.EXPAND;
        this.bits = new MyJsmpeg.BitBuffer(bufferSize, bufferMode);
    ...
    MPEG1.prototype = Object.create(MyJsmpeg.Decoder.Base.prototype);
    MPEG1.prototype.constructor = MPEG1;
    MPEG1.prototype.write = function (pts, buffers) {
        MyJsmpeg.Decoder.Base.prototype.write.call(this, pts, buffers);
    ...
    MPEG1.prototype.decode = function () {
        ...
        var elapsedTime = MyJsmpeg.Now() - startTime;
    ...
    MPEG1.prototype.decodeBlock = function (block) {
        ...
        if (this.macroblockIntra) {
            if (n === 1) {
                ...
                MyJsmpeg.Fill(this.blockData, 0)
            }
        } else {
            if (n === 1) {
...
MyJsmpeg.Decoder.MPEG1VideoWASM = function () {
    "use strict";
    var MPEG1WASM = function (options) {
        ...
        this.bufferMode = options.streaming ? MyJsmpeg.BitBuffer.MODE.EVICT : MyJsmpeg.BitBuffer.MODE.EXPAND;
        this.decodeFirstFrame = options.decodeFirstFrame !== false;
        this.hasSequenceHeader = false
    };
    MPEG1WASM.prototype = Object.create(MyJsmpeg.Decoder.Base.prototype);
    MPEG1WASM.prototype.constructor = MPEG1WASM;
    MPEG1WASM.prototype.initializeWasmDecoder = function () {
        if (!this.module.instance) {
            console.warn("MyJsmpeg: WASM module not compiled yet");
            return
        }
...
MyJsmpeg.Decoder.MP2Audio = function () {
    "use strict";
    ...
    MP2.prototype = Object.create(MyJsmpeg.Decoder.Base.prototype);
...
MyJsmpeg.Decoder.MP2AudioWASM = function () {
    "use strict";
    ...
    MP2WASM.prototype = Object.create(MyJsmpeg.Decoder.Base.prototype);
...
MyJsmpeg.Renderer.WebGL = function () {
    "use strict";
...
MyJsmpeg.Renderer.Canvas2D = function () {
    "use strict";
    ...
    CanvasRenderer.prototype.resize = function (width, height) {
        ...
        MyJsmpeg.Fill(this.imageData.data, 255)
    };
...
MyJsmpeg.AudioOutput.WebAudio = function () {
    "use strict";
    // eslint-disable-next-line no-unused-vars
    ...
    WebAudioOut.prototype.play = function (sampleRate, left, right) {
        ...
        if (this.startTime < now) {
            this.startTime = now;
            this.wallclockStartTime = MyJsmpeg.Now()
        }
    ...
    WebAudioOut.prototype.getEnqueuedTime = function () {
        return Math.max(this.wallclockStartTime - MyJsmpeg.Now(), 0)
    };
...
MyJsmpeg.WASMModule = function () {
    ...
    WASM.prototype.loadFromFile = function (url, callback) {
        ...
        var ajax = new MyJsmpeg.Source.Ajax(url);
        ajax.connect(this);
        ajax.start()
    };
    ...
    WASM.prototype.readDylinkSection = function (buffer) {
        ...
        if (!matchNextBytes([0, "a", "s", "m"])) {
            console.warn("MyJsmpeg: WASM header not found");
            return null
        }
        // eslint-disable-next-line no-redeclare
        var next = 9;
        ...
        if (!matchNextBytes([6, "d", "y", "l", "i", "n", "k"])) {
            console.warn("MyJsmpeg: No dylink section found in WASM");
            return null
        }
        return {
